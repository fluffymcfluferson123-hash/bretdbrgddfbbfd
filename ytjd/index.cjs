...
async function initDb() {
  ...
  `)
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_roadmaps_order ON roadmaps (sort_order, target_date NULLS LAST, created_at DESC)`)
...
app.use((req, res, next) => {
  ...
  res.on("finish", async () => {
    ...
    try {
      ...
      await pool.query("INSERT INTO site_events (username, path, method, status, ms, ip_hash, ua) VALUES ($1,$2,$3,$4,$5,$6,$7)", [
        username,
        pathOnly,
        method,
        status,
...
app.get("/api/roadmaps/public", async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, title, status, description, target_date, sort_order, created_at, updated_at
         FROM roadmaps
        ORDER BY sort_order ASC, target_date ASC NULLS LAST, created_at DESC
        LIMIT 200`
    )
    res.json({ items: rows })
  } catch (e) {
...
app.post("/api/roadmaps", async (req, res) => {
  if (!pool) return res.status(503).json({ error: "DB unavailable" })
  if (!isOwner(req)) return res.status(403).json({ error: "Forbidden" })
  const { title, status = "planned", description = "", target_date = null, sort_order = 1000 } = req.body || {}
  if (!title) return res.status(400).json({ error: "title required" })
  try {
    const { rows } = await pool.query(
      `INSERT INTO roadmaps (title, status, description, target_date, sort_order)
       VALUES ($1,$2,$3,$4,$5)
       RETURNING id, title, status, description, target_date, sort_order, created_at, updated_at`,
      [title, status, description, target_date, sort_order]
    )
...
app.post("/api/roadmaps/update", async (req, res) => {
  if (!pool) return res.status(503).json({ error: "DB unavailable" })
  if (!isOwner(req)) return res.status(403).json({ error: "Forbidden" })
  const { id, title, status, description, target_date, sort_order } = req.body || {}
  const nid = parseInt(id, 10)
  if (!Number.isFinite(nid)) return res.status(400).json({ error: "bad id" })
  try {
    const { rows } = await pool.query(
      `UPDATE roadmaps SET
       ...
       WHERE id=$1
       RETURNING id, title, status, description, target_date, sort_order, created_at, updated_at`,
      [nid, title ?? null, status ?? null, description ?? null, target_date ?? null, sort_order ?? null]
    )
    if (!rows.length) return res.status(404).json({ error: "not found" })
    res.json({ success: true, item: rows[0] })
  } catch (e) {
...
app.post("/api/roadmaps/delete", async (req, res) => {
  if (!pool) return res.status(503).json({ error: "DB unavailable" })
  if (!isOwner(req)) return res.status(403).json({ error: "Forbidden" })
  const { id } = req.body || {}
  const nid = parseInt(id, 10)
  if (!Number.isFinite(nid)) return res.status(400).json({ error: "bad id" })
  try {
    const r = await pool.query(`DELETE FROM roadmaps WHERE id=$1`, [nid])
...
routes.forEach(({ path: routePath, file, auth, scope }) => {
  app.get(routePath, async (req, res) => {
    ...
    if (scope) {
      const ok = await ensureAccess(req, res, scope)
      if (!ok) return
    }
